# Shrimply Smart — Task List
## Capstone Improvement Tracker
...
- [ ] Create at least 1 active Season in the database
- [ ] Enter at least 5 days of DailyGrowthMetric data (weight, count, feed, mortality)
- [ ] Verify growth predictions appear on /growth-dashboard
- [ ] Auto-fill water_temperature, water_ph, dissolved_oxygen, tds from sensor readings
      (instead of leaving them blank/manual)
- [ ] Auto-fill weather_condition from Open-Meteo weather API each day
- [ ] After first harvest: use total_harvest_kg as ML training label
- [ ] Retrain model after each completed season
- [ ] Add air temperature (from weather API) as a separate feature column
...
- [ ] Create a Season first from /history
- [ ] Enter daily shrimp data using the form
- [ ] Verify prediction chart appears after 5+ days of data
- [ ] Verify harvest date estimate appears
...
- [ ] Confirm data flows in continuously from WeMos every 5 seconds
...
- [ ] Email notification 7 days before estimated harvest date
- [ ] Auto-link feeder FeederTelemetry.distance_cm to DailyGrowthMetric.feed_amount_grams
- [ ] Collect data from multiple ponds for better ML training
- [ ] Add air temperature as a separate sensor or from weather API
